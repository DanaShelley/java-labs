    ...
    <form name="pensionerForm" action="" method="POST" class="row g-3">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="membership" id="membership" required>
                <option value="${teacher.membership.id}" name="${teacher.membership.id}">${teacher.membership.name}</option>
            </select>
        </div>

        <div class="form-group">
            <label for="school" class="form-label">School: </label>
            <input type="text" name="school" id="school" class="form-control" placeholder="Гімназія №4" required value="${teacher.school}">
        </div>

        <div class="form-group">
            <label for="lessons" class="form-label">Lessons: </label>
            <input type="text" name="lessons" id="lessons" class="form-control"
                   placeholder="Українська мова та література" required value="${teacher.lessons}">
        </div>
