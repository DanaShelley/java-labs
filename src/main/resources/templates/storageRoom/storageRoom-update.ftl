...
<head>
    <meta charset="UTF-8">
    <title>Редагувати кімнату-сховище</title>
...
<div style="width: 500px; margin: 0 auto">
    <h2>Редагувати кімнату-сховище</h2>
            ...
            <select class="form-control" name="library" id="library">
                <#list libraries as library>
                    <option value="${library.id}" name="${library.name}"
                            <#if storageRoom.library==library>selected</#if>>${library.name}</option>
                </#list>
            </select>
        </div>
