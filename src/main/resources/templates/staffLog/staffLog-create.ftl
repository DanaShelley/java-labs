...
<div style="width: 25%; margin: 0 auto">
    ...
    <form name="staffLogForm" action="" method="POST" class="row g-3">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="staff" id="staff" required>
                <#list staffs as staff>
                    <option value="${staff.id}" name="${staff.id}">${staff.name}</option>
                </#list>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="readingRoom" id="readingRoom" required>
                <#list readingRooms as readingRoom>
                    <option value="${readingRoom.id}" name="${readingRoom.id}">${readingRoom.number} - ${readingRoom.library.name}</option>
                </#list>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="date" name="startDate" id="startDate" class="form-control" value="${staffLogForm.startDate}" required>
        </div>
